...
func (r *Manager) syncToOtherClustersTest(ctx context.Context, fg *crdv1alpha1.FailoverGroup) {
	// Do nothing in test
	if r.MCReconciler == nil {
		return
	}
	// Only call the real method if MCReconciler is not nil
	r.syncToOtherClusters(ctx, fg)
}

var _ = Describe("FailoverGroup Manager", func() {
