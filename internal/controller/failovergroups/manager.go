...
// Manager reconciles a FailoverGroup object
type Manager struct {
	client.Client
...
func (r *Manager) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if ctx.Value(controller.CrdOnlyKey) != nil && ctx.Value(controller.CrdOnlyKey).(bool) {
		crdOnly = true
		logger = logger.WithValues("reconcileType", "crdOnly")
	}

	// Check if this is a sync operation
	if ctx.Value(controller.SyncModeKey) != nil && ctx.Value(controller.SyncModeKey).(bool) {
		syncMode = true
		logger = logger.WithValues("reconcileType", "sync")
	}

	// Get cluster name from context if available
	if ctx.Value(controller.ClusterNameKey) != nil {
		clusterName = ctx.Value(controller.ClusterNameKey).(string)
	...
	if clusterName != failoverGroup.Status.GlobalState.ActiveCluster && !syncMode && failoverGroup.Status.GlobalState.ActiveCluster != "" {
		...
		if err == nil {
			...
			if shouldUpdateFromActiveFG(failoverGroup, activeFG) {
				...
				syncCtx := context.WithValue(ctx, controller.SyncModeKey, true)
				syncCtx = context.WithValue(syncCtx, controller.FailoverGroupKey, activeFG)
	...
	if (clusterName == failoverGroup.Status.GlobalState.ActiveCluster || (clusterName == "" && failoverGroup.Status.GlobalState.ActiveCluster == "")) && !syncMode {
		...
		if r.MCReconciler != nil {
			...
			logger.V(1).Info("Skipping sync to other clusters, MCReconciler is nil")
		}
	}
...
func (r *Manager) syncToOtherClusters(ctx context.Context, failoverGroup *crdv1alpha1.FailoverGroup) {
	...
	for clusterName, cl := range clusters {
		...
		syncCtx := context.WithValue(ctx, controller.ClusterNameKey, clusterName)
		syncCtx = context.WithValue(syncCtx, controller.CrdOnlyKey, true) // Only sync the CRD, not actually reconcile resources
...
func (r *Manager) watchRemoteClusters(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			for name, cl := range clusters {
				...
				clusterCtx = context.WithValue(clusterCtx, controller.ClusterNameKey, name)
...
func (r *Manager) pollClusterResources(ctx context.Context, clusterName string, cl cluster.Cluster) {
...
func (r *Manager) reconcileFailoverGroup(ctx context.Context, logger logr.Logger, failoverGroup *crdv1alpha1.FailoverGroup) error {
...
func (r *Manager) reconcileFailoverGroupInCluster(ctx context.Context, logger logr.Logger, failoverGroup *crdv1alpha1.FailoverGroup, clusterName string, targetClient client.Client) error {
...
func (r *Manager) Queue(request reconcile.Request, ctx context.Context) {
...
func (r *Manager) GetCluster(ctx context.Context, logger logr.Logger, clusterName string) (cluster.Cluster, error) {
	if r.MCReconciler == nil {
...
func (r *Manager) waitForClustersReady(ctx context.Context) bool {
